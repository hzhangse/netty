...
public final class HelloWorldHttp2Handler extends Http2ConnectionHandler implements Http2FrameListener {
	...
	public void onHeadersRead(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int padding,
		...
		String path = headers.get(Http2Headers.PseudoHeaderName.PATH.value()).toString();
		RequestHandler handler = RequestHandlerFactory.getInstance().getRequestHandler(path, this.connection());
		CharSequence ack = headers.get("ack");
		
		if (path != null && ack!=null) {
			handler.handleAck(headers);
			
		}else if (path != null && ack==null)  {
			handler.handleHeaderFrame(ctx, streamId, headers, padding, endOfStream, this.encoder());
		}
	}
